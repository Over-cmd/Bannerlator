...
<p align="center">
<img src="https://img.shields.io/badge/release-3.1.2--wayland--pre5-ff3d8b?style=for-the-badge" alt="3.1.2-wayland-pre5" />
...
# Bannerlator 3.1.2 — Wayland pre-release 5

Run Windows apps and games on Android — no PC and no root required.

**Pre-release 5 of the Wayland backend: screen effects no longer cost you the zero-copy path, a window above a game gets its own display layer, and refresh-rate matching now reaches Wayland properly.** Everything from pre-release 4 is here too: the live zero-copy switch, every scaling mode and screen effect, and frame generation with LSFG Native and Win-FG Native. Wayland is a second display backend that runs games through a compositor Bannerlator carries itself. Instead of the Java X server, a game can draw through an embedded Wayland compositor that Bannerlator runs itself. It is a tester build: everything here has been proven on one device (an AYANEO Pocket FIT, Adreno 750) plus one tester's Samsung with an Adreno 8xx. It is not offered by the in-app updater and it does not replace 3.1.1 — you install it over 3.1.1 by hand, and your X11 containers keep working exactly as before.

<!-- update-summary: Wayland pre-release 5 for testers: screen effects keep hardware composition, overlay windows get their own display layer, refresh-rate matching reaches Wayland. Not offered in-app. -->
...
> ⚠️ **Read before testing.** Wayland is opt-in per container. It needs the Wayland Proton layer attached to this release (`proton-11.0-2.1-arm64ec-wayland-v6.wcp`); on any other layer the Wayland choice is greyed out and a launch falls back to X11 with a notice. Expect rough edges listed under *What it cannot do yet*.

# What is in this release (all three flavours)

| File | What it is |
|---|---|
| `Bannerlator-3.1.2-wayland-pre5-standard.apk` | the normal app (`com.winlator.banner`) |
| `Bannerlator-3.1.2-wayland-pre5-pubg.apk` | the "pubg" package name flavour (`com.tencent.ig`) |
| `Bannerlator-3.1.2-wayland-pre5-ludashi.apk` | the Ludashi package name flavour |
| `proton-11.0-2.1-arm64ec-wayland-v6.wcp` | the Wayland Proton layer (Proton 11.0-2 + winewayland + eight bundled Turnips + xkb data + the desktop-close fix), installs as **Proton-11.0-2.1-arm64ec-6** |
...
2. Install the wcp: **Contents → Proton → Install from file**. It installs as `Proton-11.0-2.1-arm64ec-6`, its own layer line, so it is never offered as an "update" to a normal 11.0-2 container. Remove the older `-1` to `-5` entries if you have them.
...
4. Container settings: Proton = `Proton-11.0-2.1-arm64ec-6`, **Display backend = Wayland**, Compositor driver = your Android Turnip, Wayland game driver = **Auto**, FEXCore = an installed version. DXVK / VKD3D, Wine components, esync and audio: the same as you use on X11.
...
**Same Proton layer as pre-release 4** (`proton-11.0-2.1-arm64ec-wayland-v6.wcp`); only the app changed.
...
**This release needs the new Proton layer** (`proton-11.0-2.1-arm64ec-wayland-v6.wcp`, installs as `Proton-11.0-2.1-arm64ec-6`): the switch is a conversation between the compositor and the game's driver, and the driver lives inside the layer.
