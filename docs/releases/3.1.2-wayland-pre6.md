...
- **OpenGL is newly working, not broadly tested.** One game has been proven to render; a game may still fail for its own reasons (the one used for this fix quits a moment later, chasing an old NVIDIA Cg shader profile that Zink does not offer). Reports from OpenGL games are very welcome.
