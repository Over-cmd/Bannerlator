...
<p align="center">
<img src="https://img.shields.io/badge/release-3.1.2--wayland--pre6-ff3d8b?style=for-the-badge" alt="3.1.2-wayland-pre6" />
...
# Bannerlator 3.1.2 — Wayland pre-release 6

Run Windows apps and games on Android — no PC and no root required.

**Pre-release 6 of the Wayland backend: native OpenGL games render for the first time.** They were always black, and the cause turned out to be ours. Everything from pre-release 5 is here too: screen effects that keep the game on its display layer, a window above a game getting its own layer, refresh-rate matching, the live zero-copy switch and frame generation. Wayland is a second display backend that runs games through a compositor Bannerlator carries itself. Instead of the Java X server, a game can draw through an embedded Wayland compositor that Bannerlator runs itself. It is a tester build: everything here has been proven on one device (an AYANEO Pocket FIT, Adreno 750) plus one tester's Samsung with an Adreno 8xx. It is not offered by the in-app updater and it does not replace 3.1.1 — you install it over 3.1.1 by hand, and your X11 containers keep working exactly as before.

<!-- update-summary: Wayland pre-release 6 for testers: native OpenGL games render on Wayland for the first time, plus everything from pre-release 5. Not offered in-app. -->
...
> ⚠️ **Read before testing.** Wayland is opt-in per container. It needs the Wayland Proton layer attached to this release (`proton-11.0-2.1-arm64ec-wayland-v7.wcp`); on any other layer the Wayland choice is greyed out and a launch falls back to X11 with a notice. Expect rough edges listed under *What it cannot do yet*.

# What is in this release (all three flavours)

| File | What it is |
|---|---|
| `Bannerlator-3.1.2-wayland-pre6-standard.apk` | the normal app (`com.winlator.banner`) |
| `Bannerlator-3.1.2-wayland-pre6-pubg.apk` | the "pubg" package name flavour (`com.tencent.ig`) |
| `Bannerlator-3.1.2-wayland-pre6-ludashi.apk` | the Ludashi package name flavour |
| `proton-11.0-2.1-arm64ec-wayland-v7.wcp` | the Wayland Proton layer (Proton 11.0-2 + winewayland + eight bundled Turnips + xkb data + the desktop-close fix), installs as **Proton-11.0-2.1-arm64ec-7** |
...
2. Install the wcp: **Contents → Proton → Install from file**. It installs as `Proton-11.0-2.1-arm64ec-7`, its own layer line, so it is never offered as an "update" to a normal 11.0-2 container. Remove the older `-1` to `-6` entries if you have them.
...
4. Container settings: Proton = `Proton-11.0-2.1-arm64ec-7`, **Display backend = Wayland**, Compositor driver = your Android Turnip, Wayland game driver = **Auto**, FEXCore = an installed version. DXVK / VKD3D, Wine components, esync and audio: the same as you use on X11.
...
**This release needs the new Proton layer** (`proton-11.0-2.1-arm64ec-wayland-v7.wcp`, installs as `Proton-11.0-2.1-arm64ec-7`): the fix is half in the app and half in the layer's graphics libraries.
...
**This release needs the new Proton layer** (`proton-11.0-2.1-arm64ec-wayland-v7.wcp`, installs as `Proton-11.0-2.1-arm64ec-6`): the switch is a conversation between the compositor and the game's driver, and the driver lives inside the layer.
...
- **Known workaround for OpenGL games that vanish without a crash:** put `GALLIUM_THREAD=0` in the game's Env Vars. Mesa's threaded-driver helper can fault on this build, and because Wine owns the crash handling the process simply disappears with no error, no log and no dump. The variable removes that helper thread and costs a little CPU-side throughput. The game used to find this bug goes from dying after two frames to playing its full intro at a steady 30 fps with it set.
...
- **Two layers costs hardware composition on this panel.** With a window above the game, this device hands the frame back to the GPU rather than composing both layers itself, and only recovers when the game's layer is rebuilt. The game still keeps its copy-free frames; other panels may behave differently and reports are welcome.
