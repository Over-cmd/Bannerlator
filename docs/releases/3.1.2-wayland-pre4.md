...
<p align="center">
<img src="https://img.shields.io/badge/release-3.1.2--wayland--pre4-ff3d8b?style=for-the-badge" alt="3.1.2-wayland-pre4" />
...
# Bannerlator 3.1.2 — Wayland pre-release 4

Run Windows apps and games on Android — no PC and no root required.

**Pre-release 4 of the Wayland backend: the zero-copy switch is now live — flip it in the in-game drawer and the game moves onto its own display layer, or back off it, without leaving the game.** Everything from pre-release 3 is here too: every scaling mode and screen effect, frame generation with LSFG Native and Win-FG Native, and a Display backend row in the Task Manager. Wayland is a second display backend that runs games through a compositor Bannerlator carries itself. Instead of the Java X server, a game can draw through an embedded Wayland compositor that Bannerlator runs itself. It is a tester build: everything here has been proven on one device (an AYANEO Pocket FIT, Adreno 750) plus one tester's Samsung with an Adreno 8xx. It is not offered by the in-app updater and it does not replace 3.1.1 — you install it over 3.1.1 by hand, and your X11 containers keep working exactly as before.

<!-- update-summary: Wayland pre-release 4 for testers: the zero-copy switch works live in-game, plus everything from pre-release 3. Not offered in-app. -->
...
| File | What it is |
|---|---|
| `Bannerlator-3.1.2-wayland-pre4-standard.apk` | the normal app (`com.winlator.banner`) |
| `Bannerlator-3.1.2-wayland-pre4-pubg.apk` | the "pubg" package name flavour (`com.tencent.ig`) |
| `Bannerlator-3.1.2-wayland-pre4-ludashi.apk` | the Ludashi package name flavour |
...
- **It still steps aside for effects and frame generation.** Turning on a Look, a scaling mode or LSFG pauses zero-copy for as long as it is needed and resumes it afterwards, live, with no relaunch.
...
4. Screen effects on GPUs whose buffers the compositor cannot import, now possible with the layer machinery the live switch added.
