...
<p align="center">
<img src="https://img.shields.io/badge/release-3.1.2-ff3d8b?style=for-the-badge" alt="3.1.2" />
...
&nbsp;<img src="https://img.shields.io/badge/layers-versionCode%208-8b5cf6?style=for-the-badge" alt="versionCode 8 layers" />
</p>
...
**3.1.2 is the Wayland release.** Instead of drawing through the Java X server, a game can now draw through an embedded **Wayland compositor** that Bannerlator runs itself — a second display backend, chosen per container. It brings things X11 could never do here: **HDR10** on HDR screens, **launching a game on your TV**, **zero-copy presentation**, **eight bundled game drivers** you can pick per game, real **mouse-look**, and frame generation in HDR. After nine tester pre-releases it is on by choice, not by default: **every existing container keeps using X11 and behaves exactly as before** until you switch it. New **versionCode 8 compatibility layers** carry the Wayland driver, so the backend lights up on whichever layer you use.

<!-- update-summary: Wayland is here: a second display backend you turn on per container, with HDR10 on HDR screens, launching games on your TV, eight bundled game drivers to pick from per game, zero-copy presentation and frame generation in HDR. New versionCode 8 compatibility layers carry it. Your existing containers keep using X11 and behave exactly as before. -->

> ⚠️ **Wayland is opt-in, per container, and it is new.** Turn it on in a container's settings; it needs a **versionCode 8** Proton layer (on an older layer the choice is greyed out and a launch falls back to X11 with a notice). HDR is off unless you turn it on and only appears on screens that report HDR10. **Wayland needs an Adreno GPU** — it uses bundled Turnip drivers, so Mali, Xclipse and PowerVR phones stay on X11, which is unchanged for them. Read *What it can and cannot do right now* below before you switch a container you care about.

> 🖥️ **The X11 vs Wayland deck: [the412banner.github.io/Bannerlator/wayland-deck.html](https://the412banner.github.io/Bannerlator/wayland-deck.html)** — where a frame goes on each backend, the measured frame rates, the root causes fixed, and what Wayland can do that X11 never could.

> 📘 **New to LSFG Native? Read the plain-language guide: [LSFG Native Made Simple](https://the412banner.github.io/Bannerlator/lsfg-native-guide.html)** — why Max FPS × multiplier has to fit your screen's refresh rate, why a steady base matters, a live checker, a cheat sheet and real-world fixes. The same rule applies to **Win-FG Native**, which always runs at 2×.
...
All the Proton / GE-Proton 11.x layers were rebuilt as **versionCode 8** (proton-wine release [`build-bionic-layers-20260917-wayland-v8`](https://github.com/The412Banner/proton-wine/releases/tag/build-bionic-layers-20260917-wayland-v8)) and are the in-app catalog default. They install into a new `<version>-arm64ec-8` slot beside v7's `-7`, so nothing is overwritten; a container already on a `-7` layer is offered this as an **Update layer → v8** on its container card, keeping games, saves and settings, with a revert snapshot.
