...
static int add_statements(struct sock_fprog *program, size_t nb_statements,
  ...
  tmp = talloc_realloc(NULL, program->filter, struct sock_filter,
                       length + nb_statements);
  if (tmp == NULL)
    return -ENOMEM;
  program->filter = tmp;

  for (i = 0; i < nb_statements; i++, length++)
    memcpy(&program->filter[length], &statements[i],
 ...
 */
static int add_trace_syscall(struct sock_fprog *program, word_t syscall,
                             int flag) {
  int status;
 ...
 * otherwise 0.
 * (From WinNative, maxjivi05, c347fac8.)
 */
...
static int start_arch_section(struct sock_fprog *program, uint32_t arch,
  ...
  /* Sanity checks.  */
  if (arch_offset > UINT32_MAX || syscall_offset > UINT32_MAX ||
...
static int merge_filtered_sysnums(TALLOC_CTX *context, FilteredSysnum **sysnums,
  ...
  for (i = 0; new_sysnums[i].value != PR_void; i++) {
    ...
    for (j = 0; (*sysnums)[j].value != PR_void &&
                (*sysnums)[j].value != new_sysnums[i].value;
         j++)
      ;
...
int enable_syscall_filtering(const Tracee *tracee) {
  ...
  status =
      merge_filtered_sysnums(tracee->ctx, &filtered_sysnums, proot_sysnums);
  if (status < 0)
    return status;
