...
void translate_syscall_exit(Tracee *tracee) {
  ...
  case PR_ioctl: {
    /* TCGETS answered a TCGETS2, see enter.c: the speeds the kernel would have
     * reported are those its c_cflag encodes.  */
...
#undef POKE_WORD
  ...
  case PR_statfs64: {
    ...
    if (read_path(tracee, statfs_path, peek_reg(tracee, MODIFIED, SYSARG_1)) <
        0) {
