...
#define _GNU_SOURCE /* process_vm_readv(2), process_vm_writev(2), */
 ...
 *
 * Android's allocator tags heap pointers, and the first guest program is started by
 * PRoot's own child, from its heap: execve(2) arrives with 0xb4... addresses for the
 * path and the environment.  The kernel takes those from the tracee, but on some
 * kernels not from its tracer - PEEKDATA answers EIO - and nothing then starts.
 * Guest addresses are never tagged, so this costs them nothing.
 * (WinNative, maxjivi05, 290c5314.)
 */
 ...
 * system call for every eight bytes, and every path PRoot translates is read and
 * written back that way.  False when the kernel did not move all of it - a page
 * that is not mapped, or not writable as ptrace(2) can force it to be - and the
 * caller then goes the slow way, which also reports the error.
 * (WinNative, maxjivi05, 0c71304a.)
 */
...
int read_string(const Tracee *tracee, char *dest_tracer, word_t src_tracee,
   ...
   * the piece that reaches it, and a string's length is not known, so it is read
   * in pieces that each end where a page might. */
  word_t offset = 0;
